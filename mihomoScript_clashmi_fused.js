/**
 * mihomo配置覆写脚本（融合版 · clashmi 轻量注入 · 方案A）
 * 底座：AIsouler / MyClash mihomoScript.js (2025-11)
 * 融合来源：cgoder/clash_rules clashmi.yml
 * 注入内容（开关可控）：
 *   1) AI 拆分：ChatGPT / Claude / Gemini 独立分组（兼容原 AI 总开关）
...
const ruleOptionsEnable = {
  ...
  // 以下为分流策略配置（原 AI 为总开关，细分开关受其控制）
  AI: true, // 国外AI服务总开关（关闭则 ChatGPT/Claude/Gemini 均关闭）
  ChatGPT: true, // clashmi 注入：OpenAI / ChatGPT
  ...
  // --- clashmi 个性化注入开关 ---
  启用大陆分组: true, // 是否生成 亚洲/欧洲/美洲/其他地区 四大洲手动组（含韩国等全量国家）
...
const continentDefinitions = [
  {
    ...
    flag: '🌏',
    regex:
      /香港|台湾|台灣|新加坡|日本|韩国|韓國|印度|泰国|泰國|马来西亚|馬來西亞|菲律宾|菲律賓|越南|印尼|印度尼西亚|🇭🇰|🇹🇼|🇸🇬|🇯🇵|🇰🇷|🇮🇳|🇹🇭|🇲🇾|🇵🇭|🇻🇳|🇮🇩|\bHK\b|\bHKG\b|\bTW\b|\bSG\b|\bSGP\b|\bJP\b|\bJPN\b|\bKR\b|\bKOR\b|\bIN\b|\bTH\b|\bMY\b|\bPH\b|\bVN\b|\bID\b|Hong|Taiwan|Singapore|Japan|Korea|India|Thailand|Malaysia|Philippines|Vietnam|Indonesia/i,
    icon: `${ICON_BASE_CLASHMI}/AS.png`,
  },
  {
    name: '欧洲',
    flag: '🇪🇺',
    regex:
      /德国|德國|英国|英國|法国|法國|荷兰|荷蘭|瑞士|意大利|義大利|西班牙|芬兰|芬蘭|瑞典|挪威|丹麦|比利时|奥地利|波兰|罗马尼亚|羅馬尼亞|捷克|葡萄牙|希腊|匈牙利|爱尔兰|俄罗斯|俄羅斯|土耳其|🇩🇪|🇬🇧|🇫🇷|🇳🇱|🇨🇭|🇮🇹|🇪🇸|🇫🇮|🇸🇪|🇳🇴|🇩🇰|🇧🇪|🇦🇹|🇵🇱|🇷🇴|🇨🇿|🇵🇹|🇬🇷|🇭🇺|🇮🇪|🇷🇺|🇹🇷|\bDE\b|\bUK\b|\bGB\b|\bFR\b|\bNL\b|\bCH\b|\bIT\b|\bES\b|\bFI\b|\bSE\b|\bNO\b|\bDK\b|\bBE\b|\bAT\b|\bPL\b|\bRO\b|\bCZ\b|\bPT\b|\bGR\b|\bHU\b|\bIE\b|\bRU\b|\bTR\b|Germany|Britain|France|Netherlands|Switzerland|Italy|Spain|Finland|Sweden|Norway|Denmark|Belgium|Austria|Poland|Romania|Czech|Portugal|Greece|Hungary|Ireland|Russia|Turkey/i,
    icon: `${ICON_BASE_CLASHMI}/EU.png`,
  },
  {
    name: '美洲',
    flag: '🌎',
    regex:
      /美国|美國|加拿大|墨西哥|巴西|阿根廷|智利|🇺🇸|🇨🇦|🇲🇽|🇧🇷|🇦🇷|🇨🇱|\bUS\b|\bUSA\b|\bCA\b|\bMX\b|\bBR\b|\bAR\b|\bCL\b|America|United States|Canada|Mexico|Brazil|Argentina|Chile/i,
    icon: `${ICON_BASE_CLASHMI}/AM.png`,
  },
  {
    name: '其他地区',
    flag: '🌍',
    regex: null, // 兜底组：动态计算（排除亚洲/欧洲/美洲）在 buildRegionGroups 中处理
    icon: `${ICON_BASE_CLASHMI}/OT.png`,
  },
];
// 用于节点匹配的扩展列表（仅含可正则匹配的大陆）
const allRegionDefinitionsWithContinent = [...allRegionDefinitions, ...continentDefinitions.filter((c) => c.regex)];
...
// --- clashmi 注入：自定义本地规则 Rule Providers（classical/text）---
...
const clashmiExtraProviders = {
  ...
  speedtest_domain: {
    ...ruleProviderCommonDomain,
    url: 'https://fastly.jsdelivr.net/gh/MetaCubeX/meta-rules-dat@meta/geo/geosite/ookla-speedtest.mrs',
  ...
  ResourceSite: {
    ...
    path: './ruleset/ResourceSite.list',
  },
  PanVod: {
    ...ruleProviderCommonClassical,
    url: 'https://v4.gh-proxy.org/https://raw.githubusercontent.com/eulac-dev/Proxy/refs/heads/main/Shadowrocket/Rules/PanVod.list',
    path: './ruleset/PanVod.list',
  },
  add_direct_domain: {
    ...ruleProviderCommonDomain,
    url: 'https://fastly.jsdelivr.net/gh/Seven1echo/Yaml/refs/heads/main/rules/Seven1_Direct_Domain.mrs',
...
const serviceConfigs = [
  ...baseGroups,
  // --- clashmi 注入：AI 拆分为 ChatGPT / Claude / Gemini（兼容原 AI 总开关）---
  {
    name: 'ChatGPT',
    baseOption: selectBaseOption,
    defaultSelected: '美国',
    ...
    icon: `${ICON_BASE_CLASHMI}/ChatGPT.png`,
    rules: ['RULE-SET,openai_domain,ChatGPT'],
  },
  {
    name: 'Claude',
    baseOption: selectBaseOption,
    ...
    icon: `${ICON_BASE_CLASHMI}/Claude.png`,
    rules: ['RULE-SET,anthropic_domain,Claude'],
  },
  {
    name: 'Gemini',
    baseOption: selectBaseOption,
    ...
    icon: `${ICON_BASE_CLASHMI}/Gemini.png`,
    rules: ['RULE-SET,google-gemini_domain,Gemini'],
  },
  {
    name: 'Media',
    baseOption: selectBaseOption,
...
function getMatchedRegions(proxyName) {
  ...
  // 融合版：同时匹配大陆分组（亚洲/欧洲/美洲），用于后续大陆组归类；缓存复用
  const regions = allRegionDefinitionsWithContinent.filter((region) => region.regex.test(proxyName));
  // 同时保留对原 allRegionDefinitions 的兼容：allRegionDefinitionsWithContinent 已包含前者
...
function buildRegionGroups(filteredProxies, customProxies) {
  ...
  const enableContinent = ruleOptionsEnable.启用大陆分组;

  // 节点分类：地区/倍率
  const regionGroups = Object.fromEntries(allRegionDefinitions.map(({ name }) => [name, []]));
  // 大陆分组：亚洲/欧洲/美洲（其他地区单独计算）
  const continentGroups = Object.fromEntries(
    continentDefinitions.filter((c) => c.regex).map(({ name }) => [name, []])
  );
  const otherProxies = [];
  const otherContinentProxies = [];

  const continentRegexes = continentDefinitions.filter((c) => c.regex).map((c) => c.regex);
  const isContinentProxy = (name) => continentRegexes.some((re) => re.test(name));
  ...
  for (const proxy of [...filteredProxies, ...customProxies]) {
    ...
    for (const region of matchedRegions) {
      if (regionGroups[region.name] !== undefined) regionGroups[region.name].push(proxy.name);
      if (continentGroups[region.name] !== undefined) continentGroups[region.name].push(proxy.name);
    }

    if (!isRegionProxy) {
      otherProxies.push(proxy.name);
    }
    // 其他地区：不在任何大陆正则中的节点（非洲/大洋洲等兜底，与“其他节点”语义不同）
    if (enableContinent && !isContinentProxy(proxy.name)) {
      otherContinentProxies.push(proxy.name);
    }
  }
  ...
  // --- clashmi 注入：大陆分组（亚洲/欧洲/美洲/其他地区）---
  if (enableContinent) {
    for (const cont of continentDefinitions.filter((c) => c.regex)) {
      const list = continentGroups[cont.name];
      if (list.length > 0) {
        generatedRegionGroups.push(...createRegionGroup(cont.name, cont.icon, list));
      }
    }
    if (otherContinentProxies.length > 0) {
      const otherDef = continentDefinitions.find((c) => c.name === '其他地区');
...
function buildFunctionalGroups(filteredProxies, generatedRegionGroups, customizeInfo) {
  ...
  if (enableClashmiLocal) {
    Object.assign(finalRuleProviders, clashmiExtraProviders);
  }

  // --- 兼容原 AI 总开关：AI=false 时强制关闭细分开关 ---
  const isServiceEnabled = (name) => {
    if (['ChatGPT', 'Claude', 'Gemini'].includes(name)) {
      return ruleOptionsEnable.AI && ruleOptionsEnable[name];
    }
    return ruleOptionsEnable[name];
  };
...
function buildDnsAndHostsConfig(config, filteredProxies) {
  ...
  // --- clashmi 注入：补充 fake-ip-filter（OrbStack / NTP / 测速等）---
  const clashmiFakeIpExtra = ruleOptionsEnable.启用自定义本地规则
    ? [
        '+.orb.local',
        'localhost',
        '*.home.arpa',
  ...
  const dns = {
    ...
    'nameserver-policy': {
      ...
      ...(ruleOptionsEnable.启用自定义本地规则 && {
        'rule-set:geolocation-!cn': foreignDNS[0],
        'rule-set:my_proxy': foreignDNS[0],
        '+.orb.local': 'system',
      }),
    },
